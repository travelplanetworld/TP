...
import { z } from 'zod';
import { DomainService, placeCreateSchema } from '@/lib/vibe/domain-service';
...
export const GET = RouteGuard.protectApi('vibe.view', async (req: NextRequest) => {
  ...
  try {
    const items = await DomainService.list('place', q.data!);
...
export const POST = RouteGuard.protectApi('places.manage', async (req: NextRequest, user) => {
  const body = await parseBody(req, placeCreateSchema);
  if (body.error) return body.error;
  try {
    const place = await DomainService.createPlace(body.data!, toActor(user));
    return ok(place, 201);
  } catch (e) { return handle(e); }
});
